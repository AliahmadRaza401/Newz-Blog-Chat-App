  ...
  Future<void> setDashboardRes(DashboardModel res) async {
    ...
    if (page == 1) {
      if (res.socialLink != null) {
        ...
        await setValue(
            PRIVACY_POLICY, res.socialLink!.privacyPolicy.toString());
        await setValue(
            TERMS_AND_CONDITIONS, res.socialLink!.termCondition.toString());
        await setValue(
            COPYRIGHT_TEXT, res.socialLink!.copyrightText.toString());
      }
      ...
      child: RefreshIndicator(
        ...
        child: Scaffold(
          body: FutureBuilder<DashboardModel>(
            initialData: getStringAsync(dashboardData).isNotEmpty
                ? DashboardModel.fromJson(
                    jsonDecode(getStringAsync(dashboardData)))
                : null,
            future: getDashboardApi(page),
            builder: (context, snap) {
              if (snap.hasData || snap.hasError) {
                DashboardModel model = snap.hasData
                    ? snap.data!
                    : DashboardModel.fromJson(
                        jsonDecode(getStringAsync(dashboardData)));
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Row(
                            children: [
                              cachedImage(ic_search,
                                      width: 22,
                                      color: Colors.grey.withOpacity(0.8))
                                  .paddingRight(8),
                              Text(
                                      appLocalization!
                                          .translate('lbl_search_news'),
                          ...
                          SearchFragment().launch(
                            ...
                            borderRadius: BorderRadius.circular(28)),
                        if (getStringAsync(featureData).isNotEmpty ||
                            model.featurePost.validate().isNotEmpty)
                          FeaturedNewsHomeWidget(
                            appLocalization: appLocalization,
                            recentNewsListing: model.featurePost ??
                                jsonDecode(getStringAsync(featureData))
                                    .map<PostModel>(
                        ...
                        FutureBuilder<List<CategoryModel>>(
                          initialData: getStringAsync(categoryData).isNotEmpty
                              ? jsonDecode(getStringAsync(categoryData))
                                  .map<CategoryModel>(
                          ...
                          builder: (context, snap) {
                            if (snap.hasData) {
                              List<CategoryModel> categoryItems = snap.hasData
                                  ? snap.data!
                                  : jsonDecode(getStringAsync(categoryData))
                                      .map<CategoryModel>(
                                          (e) => CategoryModel.fromJson(e))
                              ...
                              if (categoryItems.validate().isNotEmpty) {
                                setValue(
                                    categoryData, jsonEncode(categoryItems));
                                return Column(
                                  children: [
                                    Row(
                                      children: [
                        ...
                        if (model.recentPost.validate().isNotEmpty)
                          Column(
                            children: [
                              ...
                              NewsListViewWidget(
                                  latestNewsList: model.recentPost.validate())
                            ],
                          ),
                      ],
                    ),
